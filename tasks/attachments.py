...
def reader(lines, identifiers):
    # built here rather than in the caller: it used to be a bare name that was
    # never assigned in this scope, so every call raised NameError on the append
    attachment_cleaned = []
...
def attachmentReader(mod_folder, attachment_type):
    if attachment_type == 'mount':
        ...
        with open((Path(mod_folder)/'descr_engines.txt'), 'r', encoding="utf8") as descr_engines:
            lines = descr_engines.readlines()
    else:
        identifiers = ['type', 'rider_offset']
        with open((Path(mod_folder)/'descr_mount.txt'), 'r', encoding="utf8") as descr_mount:
